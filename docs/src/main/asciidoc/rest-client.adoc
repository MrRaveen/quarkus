...
* the `rest-jackson` extension for the REST server support. Use `rest-jsonb` if you prefer JSON-B, or `rest` if you do not need JSON support;
* the `rest-client-jackson` extension for the REST client support. Use `rest-client-jsonb` if you prefer JSON-B, or `rest-client` if you do not need JSON support.
